...
namespace TPcGameChecker
{
	class GameBlockHorizontal : Control
	{
		...
		public GameBlockHorizontal(Game g, Color color)
		{
			SetStyle(ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.SupportsTransparentBackColor | ControlStyles.UserPaint, true);
			DoubleBuffered = true;
			Size = new Size(1100, 65);
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			graph.DrawPath(new Pen(Color.White), borderPath);
			graph.FillPath(new SolidBrush(Color.White), borderPath);
			...
			graph.DrawString(GameName, Font, new SolidBrush(Color.Black), gameName, SF);
			graph.DrawString("Can you run it : " + gamec, Font, new SolidBrush(Color.Black), gameCYRI, SF);
			graph.DrawString("User rate : " + userrate.ToString(), Font, new SolidBrush(Color.Black), gameUserRate, SF);
			graph.DrawString("Metacritic : " + metacritics.ToString(), Font, new SolidBrush(Color.Black), gameMetacritics, SF);
		}
